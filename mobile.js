...
function initializePage() {
  ...
  function updateCartCount() {
    if (cartCount && cartWrapper) {
      ...
      cartWrapper.classList.toggle('has-items', state.cartItems.length > 0);
  ...
  if (pathname.includes('mobile.html') || pathname === '/') {
    setupMainPage(productPopup, updateCartCount);
  } else if (pathname.includes('cart.html')) {
    setupCartPage(updateCartCount);
  } else if (pathname.includes('favorites.html')) {
    setupFavoritesPage(updateCartCount);
...
function setupMainPage(productPopup, updateCartCount) {
  console.log('Main page logic');
  const productCards = document.querySelectorAll('.product-card');
  productCards.forEach(card => {
    card.removeEventListener('click', handleCardClick); // Удаляем старые слушатели
  ...
  document.querySelector('#popupFavorite')?.addEventListener('click', () => {
    ...
    const productId = Object.keys(state.products).find(id => state.products[id].title === popupTitle);
    if (productId) toggleFavorite(productId);
  });

  document.querySelectorAll('.product-card__button').forEach(button => {
    button.removeEventListener('click', handleAddToCart); // Удаляем старые слушатели
...
function setupCartPage(updateCartCount) {
  ...
  checkoutForm.addEventListener('submit', (e) => {
    ...
    if (name && phone && address) {
      const order = {
        ...
        phone,
        address,
        userEmail: state.currentUser ? state.currentUser.email : 'Гость'
      };
...
function setupFavoritesPage(updateCartCount) {
  ...
  toCartButton.addEventListener('click', () => {
    state.favoriteItems.forEach(item => {
      state.cartItems.push({ id: Date.now() + Math.random(), title: item.title, price: item.price, image: item.image });
    });
    localStorage.setItem('cartItems', JSON.stringify(state.cartItems));
    state.favoriteItems = [];
  ...
  function renderFavoriteItems() {
    ...
    state.favoriteItems = JSON.parse(localStorage.getItem('favoriteItems')) || [];
    toCartButton.disabled = state.favoriteItems.length === 0;
    ...
    totalElement.textContent = state.favoriteItems.length;
  }
}
...
function setupAboutPage() {
  ...
  sections.forEach(section => {
    section.removeEventListener('click', handleSectionClick); // Удаляем старые слушатели
...
function setupAccountPage() {
  ...
  if (state.currentUser) {
    ...
    accountName.textContent = state.currentUser.name;
  ...
  editProfileButton.addEventListener('click', () => {
    editName.value = state.currentUser.name;
  ...
  editForm.addEventListener('submit', (e) => {
    ...
    if (state.users.some(u => u.email === newEmail && u.email !== state.currentUser.email)) {
    ...
    state.users[userIndex].email = newEmail;
  ...
  function renderOrderHistory() {
    ...
    const userOrders = state.orderHistory.filter(order => order.userEmail === (state.currentUser ? state.currentUser.email : 'Гость'));
    if (userOrders.length === 0) {
      ordersList.innerHTML = '<li>История заказов пуста</li>';
    } else {
...
function updatePopup(product, productId) {
  ...
  const isFavorite = state.favoriteItems.some(item => item.id === productId);
...
function toggleFavorite(productId) {
  ...
  const isFavorite = state.favoriteItems.some(item => item.id === productId);
  if (!isFavorite) {
    state.favoriteItems.push({ id: productId, title: product.title, price: product.price, image: product.images[0] });
  } else {
...
function setupCarousel() {
  ...
  updateCarousel(); // Принудительно вызываем при инициализации
}
...
function setupSearch() {
  ...
  if (searchIcon && searchPopup && searchInput && searchResults) {
    searchIcon.removeEventListener('click', handleSearchIconClick); // Удаляем старый слушатель
    ...
    searchResults.addEventListener('click', (e) => {
      ...
      if (li) {
        ...
        const product = state.products[productId];
        updatePopup(product, productId);
        document.querySelector('#productPopup').setAttribute('aria-hidden', 'false');
        searchPopup.setAttribute('aria-hidden', 'true');
        searchInput.value = '';
        searchResults.innerHTML = '';
      }
...
function renderSearchResults(products) {
  ...
  if (products.length === 0) {
    ...
    products.forEach(product => {
      const li = document.createElement('li');
      li.setAttribute('data-id', Object.keys(state.products).find(id => state.products[id] === product));
...
// Инициализация при загрузке и возвращении на страницу
document.addEventListener('DOMContentLoaded', initializePage);
window.addEventListener('pageshow', (event) => {
  if (event.persisted) {
    initializePage(); // Повторная инициализация при восстановлении из bfcache
  }
});