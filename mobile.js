    ...
    if (window.location.pathname.includes('mobile.html') || window.location.pathname === '/') {
      ...
      if (searchInput && searchResults) {
        searchInput.addEventListener('focus', () => {
          ...
          const otherIcons = document.querySelectorAll('.header__extra-icons .icon-wrapper:not(.search-wrapper)');
          searchWrapper.style.width = '80%'; // Ограничим ширину
          otherIcons.forEach(wrapper => wrapper.style.opacity = '0');
        });
        ...
        searchInput.addEventListener('blur', () => {
          if (!searchInput.value) {
            const searchWrapper = document.querySelector('.search-wrapper');
            const otherIcons = document.querySelectorAll('.header__extra-icons .icon-wrapper:not(.search-wrapper)');
            searchWrapper.style.width = '33%';
            otherIcons.forEach(wrapper => wrapper.style.opacity = '1');
