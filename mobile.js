...
function initializePage() {
  ...
  // Определяем текущую страницу без учета .html
  const pathname = window.location.pathname;
  const pageName = pathname.replace('/', '').split('.')[0] || 'mobile'; // По умолчанию главная страница - 'mobile'
  ...
  if (pageName === 'favorites') {
    ...
    toCartButton.addEventListener('click', () => {
      ...
      updateCartCount();
      window.location.href = '/cart'; // Обновленный путь без .html
    });
  ...
  function setupSearch() {
    ...
    if (searchIcon && searchPopup && searchInput && searchResults) {
      searchIcon.removeEventListener('click', handleSearchClick); // Удаляем старый слушатель
...
// Запускаем инициализацию при загрузке и возвращении на страницу
