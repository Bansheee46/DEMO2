document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.pathname.includes('cart.html')) {
      ...
      checkoutForm.addEventListener('submit', (e) => {
        ...
        let address = '';
        if (deliveryType === 'pickup') {
          address = document.querySelector('input[name="pickup"]:checked').value;
        } else {
        ...
        const comment = document.querySelector('#checkout-comment').value.trim();
      ...
      function handleOrderSubmission(orderData) {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Сохраняем заказ для админки
                saveOrderForAdmin({
                    orderNumber: data.order_id || generateOrderNumber(),
                    customer_name: name + ' ' + surname,
                    customer_email: email,
                    customer_phone: phone,
                    shipping_address: address,
                    ...
                    totalPrice: cartItems.reduce((sum, item) => sum + item.price * (item.quantity || 1), 0),
                    userEmail: email
                });
            } else {
    ...
    if (window.location.pathname.includes('account.html')) {
      ...
      registerSubmit.addEventListener('submit', (e) => {
        ...
        // Добавляем пользователя в список зарегистрированных пользователей для совместимости с check-storage.js
        const registeredUsers = JSON.parse(localStorage.getItem('registeredUsers') || '[]');
        registeredUsers.push({
          name,
          email,
          hashedPassword: password, // В реальном приложении здесь должно быть хеширование пароля
          registrationDate: new Date().toISOString(),
    ...
    // ... existing code ...
      ...
      // Функция для сохранения заказа для админки
      function saveOrderForAdmin(order) {
        ...
        // Добавляем новый заказ в localStorage для админки
        ...
        // Сохраняем заказ в истории заказов пользователя
        saveOrderToHistory(order);
        
        return adminOrder;
      }
      
      // Функция для сохранения заказа в истории заказов
      function saveOrderToHistory(order) {
        // Получаем текущую историю заказов
        const orderHistory = JSON.parse(localStorage.getItem('orderHistory') || '[]');
        
        // Форматируем заказ для истории
        const historyOrder = {
        ...
        console.log('Заказ сохранен в истории:', historyOrder);
      }
